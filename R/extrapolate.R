...
#' @param mod a model that with a \code{\link[broom]{tidy}} method
#' @param col_name name to match the
#' name of the effects
...
extrapolate_effects <- function(mod, col_name, ...) {
  fixed_effects <- pull_effects(mod, col_name, ...)
  lm(paste("estimate ~ ", col_name), data = fixed_effects)
}

#' @rdname extrapolate_effects
#' @export

pull_effects <- function(mod, col_name, ...) {
  out <- mod %>%
    broom::tidy() %>%
...
augment_future <- function(mod, newdata, col_name, ...) {
  ...
  effects <- pull_effects(mod, col_name) %>%
    dplyr::pull(estimate)
  ...
  mod_effects <- extrapolate_effects(mod, col_name)
  ...
  newdata <- newdata %>%
    ...
    dplyr::left_join(
      ...
      by = c("temp" = col_name)) %>%
    dplyr::mutate(.fitted = y_hat_old + effect - max_effect)
